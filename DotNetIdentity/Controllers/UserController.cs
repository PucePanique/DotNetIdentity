...
namespace DotNetIdentity.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Register(SignUpViewModel viewModel)
        {
            if (ModelState.IsValid)
            {
                var user = new AppUser
                {
                    ...
                    Department = viewModel.userType.ToString(),
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    if (user.IsLdapLogin == true)
                    {
                        ...
                        try
                        {
                            ldapAuthSuccess = await this.makeLDAPAuth(user, viewModel.Password);
                        }
                        catch (Exception ex)
                        {
        ...
        private async Task<IEnumerable<string>> GetGroupsForUserCoreAsync(LdapConnection _ldapConnection, string user, string LDAPBaseDn)
        {
            var searchQueueTask = _ldapConnection.SearchAsync(
                LDAPBaseDn,
            ...
            var searchQueue = await searchQueueTask; // Await the Task to get the actual LdapSearchQueue object
            ...
            {
                LdapAttribute attr = entry.Get("memberOf"); // Corrected method to retrieve a single attribute
        ...
        /// <returns></returns>
        private async Task<Stack<string>> createGroupsTableAsync(LdapConnection lc, string username, string LDAPBaseDn)
        {
            // check for group membership
            var groups = new Stack<string>();
            var uniqueGroups = new HashSet<string>();

            var groupList = await this.GetGroupsForUserCoreAsync(lc, username, LDAPBaseDn); // Await the Task to get the IEnumerable<string>
            foreach (string group in groupList) // Corrected to iterate over the resolved IEnumerable<string>
            {
                groups.Push(group);
            }
            return groups;
        ...
        private async Task<bool> makeLDAPAuth(AppUser _usr, string userpw)
        {
            ...
            var sdn = await lc.GetSchemaDnAsync();

            // authenticate to the server
            await lc.BindAsync(ldapVersion, loginDN, password1);
            string authDN = lc.AuthenticationDn.ToString();

            if (authDN.ToString().Contains(loginDN))
            {
                var gr = await createGroupsTableAsync(lc, _usr.UserName!, _sett.Ldap.LdapBaseDn!); // Await the Task to resolve the Stack<string>
                ...
                {
                    // ldap auth success but user not in required group
                    lc.Disconnect();
                    return false;
                }
            }
